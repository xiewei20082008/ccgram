...
- Fix `functions -c` "unknown option" error on fish 4.5 — use `functions --copy` long form
- Shell prompt marker setup is now invisible — embedded `clear` hides the setup command
- Idempotent prompt markers for all shells — skip re-setup when already configured (fish: `functions -q` guard, bash: `type` guard, zsh: PROMPT pattern guard)
- Use `raw=True` for prompt setup `send_keys` — bypass TUI vim detection and 500ms delay

### Added

- POSIX sh/dash/ksh shell support — static `⌘0⌘` prompt marker as fallback when dynamic exit code expansion is unavailable
